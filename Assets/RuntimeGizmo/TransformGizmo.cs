...
namespace RuntimeGizmos
{
	...
	public class TransformGizmo : MonoBehaviour
	{
		...
		void OnEnable()
		{
            ...
            {
                Shader lineShader = Shader.Find("Custom/Lines");
                if (lineShader != null)
                {
                ...
                {
                    Debug.LogError("Could not find 'Custom/Lines' shader! Check the shader filename and internal name.");
                }
            }
        ...
        {
            // Only draw in the Game View or Scene View
            if (camera == myCamera || camera.name == "SceneCamera")
            {
                DrawRuntimeGizmos();
            }
        }
		...
		void LateUpdate()
		{
            ...
            //transform.position = mainTargetRoot.transform.position;
		...
		{
            if (mainTargetRoot == null || manuallyHandleGizmo || lineMaterial == null) return;

            // Apply the shader pass
            lineMaterial.SetPass(0);

            GL.PushMatrix();
            // Load the camera's projection so the 3D arrows appear in the right place
            GL.LoadProjectionMatrix(myCamera.projectionMatrix);
            GL.modelview = myCamera.worldToCameraMatrix;
            ...
            // Draw the lines, triangles, and planes
		...
		IEnumerator TransformSelected(TransformType transType)
		{
			...
			while(!Input.GetMouseButtonUp(0))
			{
                ...
                if (mousePosition == Vector3.zero || Vector3.Distance(mousePosition, originalPivot) > 500f)
                {
                ...
                if (previousMousePosition != Vector3.zero && mousePosition != Vector3.zero)
				{
					if(transType == TransformType.Move)
					{
                        ...
                        // Only translate if there is actual movement
                        if (movement.sqrMagnitude > 0)
                        {
                            for (int i = 0; i < targetRootsOrdered.Count; i++)
                            {
                                targetRootsOrdered[i].Translate(movement, Space.World);
                            }
                        }
                        for (int i = 0; i < targetRootsOrdered.Count; i++)
						{
                        ...
                        // FIX: Update the internal pivot to match the food's actual position
                        if (targetRootsOrdered.Count > 0)
                        {
                            // 1. Get where the food is AFTER the translation (and collision check)
                            Vector3 actualFoodPosition = targetRootsOrdered[0].position;

                            // 2. Calculate the difference between where the pivot was and where the food is
                            Vector3 pivotCorrection = actualFoodPosition - pivotPoint;

                            // 3. Update the internal gizmo logic so the handles stay on the food
                            SetPivotPointOffset(pivotCorrection);

                            // 4. Update the local variable used for plane intersection calculations
                            originalPivot = pivotPoint;
                        }
                    }
